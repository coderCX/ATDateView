...
- (void)drawRect:(CGRect)rect
{
    if (self.frame.size.height<216 || self.frame.size.width<320)
        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width, 216);
    ...
    if (self.minLimitDate) {
        ...
        self.minLimitDate = [self dateFromString:@"195001010000" withFormat:@"yyyyMMddHHmm"];
        minDateModel = [[UUDatePicker_DateModel alloc]initWithDate:self.minLimitDate];
    }
...
- (NSArray *)getNowDate:(NSDate *)date
{
    ...
    if (self.datePickerStyle == UUDateStyle_MonthDay)
        return @[month,day];
